...
fn get_color_line<'a, 'b>(piece: &'a Piece, color: &'a Color) -> Option<&'a Line> {
...
fn check_solved(piece: &Arc<RwLock<Piece>>, color: Color, start_or_end: LinePoints) -> bool {
    ...
    if let Some(linked_piece) = linked {
      ...
      if let Some(linked_face) = &read_linked_piece.borders[((line.sides[start_or_end as usize] + 3) % 6) as usize] {
        if linked_face.read().unwrap().id == read_piece.id {
          if read_piece.passed == true {
            true
          } else {
            {
